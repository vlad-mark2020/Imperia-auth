...
exports.logincc = async (req, res) => {
  const { token } = req.query;  // токен из query строки
  ...
  try {
    ...
    await AuthSystem.findOneAndUpdate(
    ...
    res.json({ 
      msg: "Login by token successful", 
      username: user.username 
    });
  } catch (err) {
    console.error("LoginCC Error:", err);
    res.status(500).json({ msg: "Internal server error" });
  }
};

const AuthSystem = require('../models/AuthSystem'); // подключи модель, если она есть
...
exports.login = async (req, res) => {
  ...
  const isMatch = password === user.password; // Без хеширования
  if (!isMatch) {
  ...
  // Записываем lastActive в коллекцию auth_system
  await AuthSystem.findOneAndUpdate(
    { username },
    { lastActive: currentDate },
    { upsert: true, new: true }
